    ...
    */
    ...
    Dataset.aggregate([ { "$match": { "createdBy": req.email } },{$group : {_id :{"belongsto":"$belongsTo","Status":"$Status"}, count : {$sum : 1}}}],(err,data)=>{
        ...
        else  
        {
            ...
            // itearing throught  the maindata to add element into the chart array
            maindata.forEach((obj)=>{
                labels.push(obj.title);
                complete.push(obj.completed);
                rejected.push(obj.rejected);
                incomplete.push(obj.incomplete);
            })
